...
export function buildToolPreprompt(tools: OpenAiTool[], timezone?: string): string {
	...
	return [
		...
		`HF HUB: Use Hugging Face Hub tools for Hub-specific queries. Use "Spaces Semantic Search" to find AI apps/Spaces. Use "Papers Semantic Search" for ML research papers. Use "Repository Search" to find models, datasets, and spaces with filters. Use "Hub Repository Details" for detailed info about a specific model, dataset, or space. Use "Documentation Semantic Search" for Hugging Face documentation. Prefer these over general web search when the user asks about Hugging Face models, datasets, spaces, or documentation.`,
		`WEB SEARCH: For general web search, news, or non-HF topics, use web search tools (Tavily, Exa, Jina). Use Tavily for general web search and content extraction. Use Jina's "search_arxiv" for academic papers when HF Papers search is not sufficient.`,
		`PRIORITY: When a question can be answered by either HF Hub tools or web search, prefer HF Hub tools for HF-specific content and web search for everything else.`,
