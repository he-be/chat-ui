<script lang="ts">
	...
	const isSelfHosted = !page.data.publicConfig?.isHuggingChat;

	let activeMimeTypes = $derived(
		Array.from(
			new Set([
				...TEXT_MIME_ALLOWLIST,
				...(isSelfHosted ? [...PDF_MIME_ALLOWLIST] : []),
				...(modelIsMultimodal
			...
			>
				...
				{:else}
					...
					>
						...
						{:else}
							<ChatInput
								placeholder={isReadOnly ? "This conversation is read-only." : "偽Geminiに相談"}
								{loading}
								bind:value={draft}
								bind:files
